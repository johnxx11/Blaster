...
{
	ABlasterCharacter* BlasterCharacetr = Cast<ABlasterCharacter>(OtherActor);
	if (BlasterCharacetr)
	{
		BlasterCharacetr->SetOverlappingWeapon(this);
	}
}
...
void AWeapon::OnShpereEndOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex)
{
	...
	if (BlasterCharacetr)
	{
		BlasterCharacetr->SetOverlappingWeapon(nullptr);
	}
}
